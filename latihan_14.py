...
def hough_lines_acc(img, rho_resolution=1, theta_resolution=1):
    ...
    #inisialisasi hough accumulator sesua dengan ukuran rho dan theta
...
# deteksi puncak dengan cara lebih baik
def hough_peaks(H, num_peaks, thresshold=0, nhood_size=3):
    indicies = []
    H1 = np.copy(H)
    for i in range(num_peaks):
        ...
        if ((idx_y + (nhood_size/2) + 1) > H.shape[1]): max_y = H.shape[1]
